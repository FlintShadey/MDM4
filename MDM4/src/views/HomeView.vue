<template>
  <v-container>
    ...
    ========================== -->
    <v-row>
      <!-- Display an image for the Head Injury section -->
      <img
      ...
      <!-- Button for "Head Injury No CT" -->
      <v-col>
        <v-btn
          :ref="'Head Injury No CT'"
      ...
      <!-- Button for "Pedi TBI Algorithm" (grey color kept as-is) -->
      <v-col>
        <v-btn
          :ref="'Pedi TBI Algorithm'"
          color="grey"
          size="small"
      ...
      <!-- Button for "Severe TBI" (grey color kept as-is) -->
      <v-col>
        <v-btn
          :ref="'Severe TBI'"
          color="grey"
          size="small"
          @click="openDialog('/severeTBI')"
        >
          Severe TBI
        </v-btn>
      </v-col>
    ...
    <v-row class="small-vertical">
      ...
      <v-col>
        <v-btn
          :ref="'Subconjunctival Hemorrhage'"
    ...
    <v-row>
      ...
      <v-col>
        <v-btn
          :ref="'Hand Sprain'"
          :color="getButtonColor('Hand Sprain')"
          size="small"
          rounded="xl"
          @click="openDialog('/handsprain')"
        >
          Hand Sprain
        </v-btn>
      </v-col>
    ...
    <v-row>
      ...
      <v-col>
        <v-btn
          :ref="'Swallow FB'"
          :color="getButtonColor('Swallow FB')"
          size="small"
          rounded="xl"
          @click="openDialog('/swallowedfb')"
        >
          Swallow FB
        </v-btn>
      </v-col>
...
export default {
  ...
  methods: {
    ...
    // Call this method when a search term is received.
    highlightButton(searchTerm) {
      const term = searchTerm.toLowerCase().trim();
      if (!term) return;
      for (const key in this.$refs) {
        if (key.toLowerCase().includes(term)) {
          let btnRefs = this.$refs[key];
          if (!Array.isArray(btnRefs)) {
            btnRefs = [btnRefs];
          }
          btnRefs.forEach((btn) => {
            const el = btn.$el;
...
<style scoped>
/* Add a highlight style (adjust as needed) */
.highlight {
  border: 5px solid rgb(220, 244, 3);
